...
def main(
    ...
    elif len(input_mask_files) >= num_frames:
        use_motion_seg = "png"
        print(f"Found {len(input_mask_files)} input mask PNGs. Using input masks for motion segmentation.")
    else:
        use_motion_seg = None
    ...
    for i in tqdm(range(num_frames), desc="Processing Frames"):
        ...
        if use_motion_seg == "npy":
            ...
            # 用颜色匹配找到每个 PLY 点对应的 mask 像素
            # PLY 点是按推理分辨率（320x448）的扫描顺序保存的
            pts_cols_u8 = (np.asarray(pcd.colors) * 255).astype(np.uint8)  # (N, 3) RGB

            # 读取对应帧的原始视频（需要从 ply_dir 推断视频路径）
            # 假设视频在 ply_dir.parent.parent / "input_copy.mp4"
            ...
            if not video_path.exists():
                ...
                if not ret:
                    ...
                    # resize 到推理分辨率（320x448）
                    frame_resized = _cv2.resize(frame, (448, 320))
                    frame_rgb = _cv2.cvtColor(frame_resized, _cv2.COLOR_BGR2RGB)
                    frame_flat = frame_rgb.reshape(-1, 3)  # (143360, 3)

                    # 用颜色匹配找每个 PLY 点对应的像素索引
                    from collections import defaultdict
                    color_to_idx = defaultdict(list)
                    for idx, c in enumerate(frame_flat):
                        color_to_idx[tuple(c)].append(idx)

                    pixel_indices = np.zeros(len(pts_cols_u8), dtype=np.int32)
                    for j, c in enumerate(pts_cols_u8):
                        key = tuple(c)
                        if key in color_to_idx and color_to_idx[key]:
                            pixel_indices[j] = color_to_idx[key].pop(0)
                        else:
                            pixel_indices[j] = 0
                    ...
                    dyn_mask = (mask_flat[pixel_indices] > 127).astype(np.float32)
        else:
            dyn_mask = np.ones(len(pcd.points), dtype=np.float32)
