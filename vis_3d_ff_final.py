...
def main(
    ...
    for i in tqdm(range(num_frames), desc="Processing Frames"):
        ...
        if use_motion_seg == "npy":
            ...
            # 用最近邻颜色匹配（允许容差）对齐 mask
            import cv2 as _cv2

            # 读取 frame PLY
            frame_pcd = _safe_read_ply(frame_ply_files[i])
            frame_cols = np.asarray(frame_pcd.colors) * 255  # (N, 3) float
            ...
            if not ret:
                ...
                frame_flat_rgb = frame_rgb.reshape(-1, 3).astype(np.float32)  # (143360, 3)
                ...
                print(f"Frame {i}: matched {(distances < 5).sum()}/{len(frame_cols)} points (dist<5), dynamic={frame_dyn_mask.sum():.0f}")

                # 传递给 flow PLY
                flow_cols = np.asarray(pcd.colors) * 255
                frame_color_to_dyn = {}
                for j, c in enumerate(frame_cols):
                    # 用整数 RGB 作为 key（容差匹配后取整）
                    key = tuple(np.round(c).astype(int))
                    frame_color_to_dyn[key] = frame_dyn_mask[j]

                dyn_mask = np.zeros(len(flow_cols), dtype=np.float32)
                for j, c in enumerate(flow_cols):
                    key = tuple(np.round(c).astype(int))
                    dyn_mask[j] = frame_color_to_dyn.get(key, 0.0)
        else:
            dyn_mask = np.ones(len(pcd.points), dtype=np.float32)

        # 4. Outlier Removal（对动态点放宽要求）
        # 动态点因为运动会被误判为 outlier，所以分开处理
        is_dynamic_frame = dyn_mask > 0.5

        if is_dynamic_frame.sum() > 0:
            # 动态点：放宽 outlier removal 参数
            pcd_dynamic = pcd.select_by_index(np.where(is_dynamic_frame)[0])
            _, ind_dyn = pcd_dynamic.remove_statistical_outlier(nb_neighbors=10, std_ratio=5.0)
            valid_dyn_indices = np.where(is_dynamic_frame)[0][ind_dyn]
        else:
            valid_dyn_indices = np.array([], dtype=int)

        if (~is_dynamic_frame).sum() > 0:
            # 静态点：正常 outlier removal
            pcd_static = pcd.select_by_index(np.where(~is_dynamic_frame)[0])
            _, ind_static = pcd_static.remove_statistical_outlier(nb_neighbors=30, std_ratio=2.0)
            valid_static_indices = np.where(~is_dynamic_frame)[0][ind_static]
        else:
            valid_static_indices = np.array([], dtype=int)

        current_indices = np.concatenate([valid_dyn_indices, valid_static_indices])
